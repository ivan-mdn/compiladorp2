...
public   class Util {
	...
	public static Integer obterEstadoInicial(String strTransicoes){
		...
		Log("\nEstado Inicial obtido:"+inicial.toString());
		return inicial;
	}
	public static Integer[] obterEstadosFinais(String strTransicoes){
		...
		if (log){
			...
			str.append("}");
			Log("\nEstados Finais obtidos:"+str.toString());
		}
...
//			Log(estadoInicial);
//			Log(entrada);
			Log("\nTransicao Lida - estado inicial:"+estadoInicial+" entrada:"+entrada+" proximo estado:"+proximoEstado);
		...
		if(log){
			Log("\nTransicoes reconhecidas:");
			TransicaoSintatica t = null;
			for (int i = 0;i<listaDeTransicoes.tamanho;i++){
				t = (TransicaoSintatica) listaDeTransicoes.get(i);
				Log("\nTipo:"+((t.transicaoParaMaquina)?"Maquina ":"Terminal") +"  Estado Inicial:"+t.estadoInicial.toString()+" Entrada:" +((t.transicaoParaMaquina)?t.maquina.toString():t.token.toString()) +"  Proximo Estado:"+t.proximoEstado.toString());
			}
		}
